	...
	return (
		<div className="typography-control-wrapper">
			<div className="typography-btn-control">
				<p>{__('Typography', 'hazrath-advanced-cta-block')}</p>	
			...
			{isOpen && (
				<Modal
					title={__('Typography Settings', 'hazrath-advanced-cta-block')}	
					onRequestClose={() => setOpen(false)}
					className="typography-modal"
				>
					<PanelRow>
						<SelectControl
							label={__('Font Family', 'hazrath-advanced-cta-block')}
							value={fontFamily}
							options={[
								{ label: 'Default', value: '' },
					...
					<PanelRow>
						<RangeControl
							label={`${__('Font Size', 'hazrath-advanced-cta-block')} (${device})`}
							value={fontSize}
					...
					<PanelRow>
						<SelectControl
							label={__('Font Weight', 'hazrath-advanced-cta-block')}
							value={fontWeight}
							options={[
								{ label: 'Default', value: '' },
					...
					<PanelRow>
						<SelectControl
							label={__('Text Transform', 'hazrath-advanced-cta-block')}
							value={textTransform}
							options={[
					...
					<PanelRow>
						<RangeControl
							label={`${__('Line Height', 'hazrath-advanced-cta-block')} (${device})`}
							value={lineHeight}
					...
					<Flex justify="space-between" style={{ marginTop: '20px' }}>
						...
						<FlexItem>
							<Button isPrimary onClick={() => setOpen(false)}>
								{__('Apply & Close', 'hazrath-advanced-cta-block')}
							</Button>
						</FlexItem>
					</Flex>
				</Modal>
